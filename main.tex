...
Shema Dolph NSENGIYUMVA & 222009353 & \hrulefill \\[0.7cm]

Louis Clement ABAYO & 222004020 & \hrulefill \\
\end{tabular}
...
The rapid evolution of fifth-generation (5G) wireless networks necessitates the simultaneous support of three heterogeneous service classes: Enhanced Mobile Broadband (eMBB), Ultra-Reliable Low-Latency Communications (URLLC), and Massive Machine-Type Communications (mMTC). Each service class exhibits vastly different Quality of Service (QoS) requirements in terms of data rate, reliability, and latency, making resource allocation a complex and critical challenge.

This project proposes a QoS-aware power allocation framework for 5G network slicing under Nakagami-m fading, which generalizes the widely used Rayleigh fading assumption and provides a more accurate characterization of radio propagation in dense urban environments such as Kigali, Rwanda. The system considers an uplink Heterogeneous Non-Orthogonal Multiple Access (H-NOMA) scenario where an eMBB user and a URLLC user share the same radio resource block, with Successive Interference Cancellation (SIC) employed at the base station.

Closed-form analytical expressions for the outage probability of both services are derived using the Gamma distribution properties of the Nakagami-m channel power gain. A double-summation closed-form solution for the URLLC outage probability under composite interference and fading is established. An M/D/1 queueing model is adopted to characterize the URLLC packet delay. The analytical results are used to formulate a constrained optimization problem that seeks to maximize total system throughput subject to per-slice QoS constraints: URLLC outage below \(10^{-5}\), URLLC packet delay below 1 ms, and eMBB throughput above 10 Mbps.

Simulation results, validated against Monte Carlo methods, demonstrate that the Nakagami-m model with parameter \(m > 1\) consistently yields lower outage probabilities compared to Rayleigh (\(m = 1\)), confirming the conservative nature of Rayleigh-based designs. The proposed H-NOMA framework with optimal power allocation achieves significant performance gains over conventional H-OMA, particularly when the URLLC channel quality exceeds that of eMBB.
\clearpage
...
\addcontentsline{toc}{section}{List of Abbreviations, Acronyms and Symbols}
\begin{description}
  \item[eMBB] Enhanced Mobile Broadband
  \item[URLLC] Ultra-Reliable Low-Latency Communication
  % TODO: Add the rest of your abbreviations/acronyms/symbols.
\end{description}
\clearpage
